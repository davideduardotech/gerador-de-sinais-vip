...
def datetime_and_weekday_in_string(): 
...
def logging(info, message):
    print(f"{Fore.LIGHTBLACK_EX}[LOG]{Fore.RESET}{info} {Fore.LIGHTBLACK_EX}{datetime_and_weekday_in_string()}{Fore.RESET} {message}")


def gerar_configuracao(configuracoes, timeframe=None):
        """
...

        {
            ...
            "timeframe": "5 minutos", 
            "periodo de catalogação em dias": "9 dias", 
            "martingale": "1 martingale", 
            "porcentagem de assertividade(nenhum martingale)": "99%", 
            "porcentagem de assertividade(1 martingale)": "70%", 
            "porcentagem de assertividade(2 martingale)": "70%", 
            "quantidade de operações que a Machine Learning ira filtrar": 2
        }
        """
...
def filtrar_operacoes(catalogacao, configuracoes, quantidade_de_operacoes):
        """
...
def filtrar_operacoes_por_horario(catalogacao_organizada, configuracoes):
    """
    ...
    print(f"Operações filtradas entre {hora_atual} e {proxima_hora}: {len(lista_operacoes)} encontradas.")
    return lista_operacoes

def cataloga(api_iqoption, par, dias, timeframe):
        data = []
        datas_testadas = []
        time_ = time.time()
        ...
        while sair == False:
            try:
                velas = api_iqoption.get_candles(par, (timeframe * 60), 1000, time_)
        ...
        CODDING: Log
        """
        print(f"{Fore.LIGHTBLACK_EX}[LOG]{Fore.RESET}{Fore.GREEN}[CATALOGAÇÃO]{Fore.RESET} "+"{}{}{} ativo catalogado: {} | {} dias{}(demorou {} segundos){}".format(Fore.LIGHTBLACK_EX,datetime_and_weekday_in_string(),Fore.RESET,par, dias, Fore.LIGHTBLACK_EX,abs(end_timer-start_timer),Fore.RESET))
        return analise

def catalogar_operacoes_rapidas(api_iqoption,configuracoes):
    ativos = api_iqoption.get_all_open_time()
    start_time_all = time.time()
    logging(f"{Fore.GREEN}[CATALOGAÇÃO]{Fore.RESET}", "catalogação iniciada pra filtrar novas operações...")
    ...
    for par in ativos['digital']:
        if ativos['digital'][par]['open'] == True:
            ...
            try:
                catalogacao.update({par: cataloga(api_iqoption,par, int(configuracoes['periodo de catalogação em dias'].split(' ')[0]), int(configuracoes['timeframe'].split(' ')[0]))})
...
def organizar_por_horario(catalogacao):
...
def listar_operacoes_do_horario(operacoes):
    for ativo in operacoes:  # Itera sobre cada ativo nas operações
        for horario in operacoes[ativo]:  # Itera sobre cada horário do ativo
            # Dados básicos
            dir = operacoes[ativo][horario]['dir']
            perc = operacoes[ativo][horario]['%']
            
            # Verifica se 'mg1' e 'mg2' existem
            mg1_perc = operacoes[ativo][horario].get('mg1', {}).get('%', '-')
            mg2_perc = operacoes[ativo][horario].get('mg2', {}).get('%', '-')
            
            # Imprime no formato solicitado
            print(f"{ativo} {horario} 1M {dir} mg0({perc}) mg1({mg1_perc}) mg2({mg2_perc})")

def ordenar_e_filtrar_operacoes(lista, timeframe=1):
    """
...
#configuracoes = gerar_configuracao({}, '1 minuto')
# configuracoes["timeframe"]
# configuracao["martingale"]
# configuracao["periodo de catalogação em dias"]
# configuracao["porcentagem de assertividade(nenhum martingale)"]
# configuracao[""porcentagem de assertividade(1 martingale)""]


#print('configuracoes:',configuracoes)

#operacoes = catalogar_operacoes_rapidas(api_iqoption, configuracoes)
#operacoes_organizadas = organizar_por_horario(operacoes)
#listar_operacoes_do_horario(operacoes_organizadas['07:00'])

#lista_de_sinais = filtrar_operacoes_por_horario(operacoes_organizadas, configuracoes)
#print('lista de sinais deseroganizada:', lista_de_sinais)

#lista_de_sinais_ordenada = ordenar_e_filtrar_operacoes(lista_de_sinais)
#print('lista de sinais organizada:', lista_de_sinais_ordenada)
#print('operacoes filtradas:', json.dumps(operacoes, indent=4, ensure_ascii=False))

#resposta = cataloga(api_iqoption, "EURUSD-OTC", 8, 1)
